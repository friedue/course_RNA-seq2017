...
coef(modelling_snf2)
```
The linear model is base on this function:
...
P <- ggplot(snf2, aes(x = genotype, y = rlog, fill = genotype)) + 
  ...
  ggtitle("Rlog-transformed values for snf2") + coord_cartesian(ylim = c(0, max(snf2$rlog)+1))
...
- residuals = difference between the observed values (our expression values) and the ones obtained by the model when the betas take a certain value
